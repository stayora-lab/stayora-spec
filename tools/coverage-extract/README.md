...
`guardrails.yml` is a human-maintained YAML 1.2 file written in JSON syntax, so Python's standard library can parse it. It has the 21 exact canonical row names, expected prototype source files and lists of case-insensitive substring patterns. Prefix a pattern with `re:` for a case-insensitive regular expression. A row may add a `pair_with` list: a test matches the row only if it matches a pattern and its name also contains at least one `pair_with` term; exported function names are matched by the patterns alone. A pattern changes which evidence is surfaced; it does not determine coverage. Review the unmapped section on every run because it exposes prototype behavior outside the current mapping.
